...
<div class="container">
<form role = "form" method = "POST" action = "my_uploads.php">
<div class = "row">
...
<div class="wow bounceInRight center">
<input type = "text" class = "form-control" name = "search_val" placeholder="ID/Title/Filename" />
</div></div>
<div class = "col-sm-2">
<div class="wow bounceInDown">
<select id="sel1" class = "form-control" name = "field">
  ...
  if(isset($_POST['submit']))
{
  ...
  $ord = $_POST['order'];
  if($ord == "AESC")
    $query = "SELECT id,username,title,filename,date_s FROM uploads WHERE username = '$username' ORDER BY $criteria ";
  else
    $query = "SELECT id,username,title,filename,date_s FROM uploads WHERE username = '$username' ORDER BY $criteria DESC";

  $result = $connect->query($query);
  echo '<table class="table table-striped table-bordered table-hover">'; 
   echo "<tr><th>ID</th><th>Title</th><th>Filename</th><th>Date</th></tr>"; 
...
if(!empty($_POST['search_val']))
{
  ...
  $field = $_POST['field'];
  $query  = "SELECT * FROM uploads WHERE $field = '$search' AND username = '$username'";
  $result = $connect->query($query);
...
if($flag == 0){
  ...
  $query = "SELECT id,username,title,filename,date_s FROM uploads WHERE username = '$username'";
  $result = $connect->query($query);
  echo '<table class="table table-striped table-bordered table-hover">'; 
    echo "<tr><th>ID</th><th>Title</th><th>Filename</th><th>Date</th></tr>"; 
  while($row = $result->fetch_assoc())
